...
def Connection( ):
    return pyodbc.connect(
        'DRIVER={ODBC Driver 13 for SQL Server};' + 'SERVER=localhost;' + 'PORT=1443;' + 'DATABASE=StockDB;' + 'UID=sa;' + 'PWD=admin;' )


# Create the sqlalchemy engine using the pyodbc connection
engine = create_engine( "mssql+pyodbc://?driver=ODBC+Driver+13?charset=utf8", creator = Connection,
...
class DB_TechAnalysis:
    ...
    def CompareDB( self, data ):
        ...
        del left[ '成交量_資料庫取出' ]
        self.df = left  # self.df = self.df[ ~self.df[ '日期' ].isin( lst ) ]  #  print( data, '刪除重覆寫入', self.df )

    def ReadCSV( self, file ):
        self.df = pd.read_csv( file, sep = ',', encoding = 'utf8', false_values = 'NA', dtype = { '日期': str } )
        self.df = self.df.replace( [ np.inf, -np.inf ],
            np.nan )  # self.df[ '日期' ] = pd.to_datetime( self.df[ '日期' ], format = "%y%m%d" )  # print( self.df )
...
def main( ):
    ...
    stock_d = { '分': [ db_H, '_60分線技術指標.csv' ], '日': [ db_D, '_日線技術指標.csv' ], '周': [ db_W, '_周線技術指標.csv' ],
                '月': [ db_M, '_月線技術指標.csv' ] }
